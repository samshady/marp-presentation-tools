...
- **RULE-CF-02:** Bullet spacing is handled by the theme's `::before` content: `content: "– "` (en dash + space). Do not add inline `padding-left` or `margin-left` overrides on `ul` or `li::before` in scoped styles — they cannot override theme pseudo-element selectors.
